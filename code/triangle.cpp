...
void Triangle::glDraw ()
{
    ...
    */
    ...
    if (options->textures && textured && material->textured())
    {
        ...
        // n is the face normal for the triangle. t are the texture coordinates. v are the vertex positions. (declared in triangle.h)
        // glLoad loads the material properties into OpenGL (from material.h)
        n.glLoad();
        t[0].glLoad();
        v[0].glLoad();
        t[1].glLoad();
