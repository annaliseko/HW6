...
void menu_file_open ()
{
    ...
    if (sourceFile.good())
    {
        scene->setFile(options->source.c_str());
        sourceFile >> *scene;
        sourceFile.close();
    }
    else
...
void menu_file_save_image ()
{
    ...
    if (cin.fail())
    {
        cerr << "ERROR: cin unable to read filename" << endl;
        return;
    }
    ...
    if (image->write(options->destination) == -1)
    {
        ...
        cerr << "     required extension: .bmp " << endl;
    }
...
// What does jitter do and why do we need it in our ray tracer?
