...
int main (int argc, char** argv)
{
    ...
    glutCreateWindow("CS155 Ray Tracer");
...
void init (int argc, char** argv)
{
    ...
    // random number generator used for perlin noise generation in perlin.cpp
    srand48(time(0));
    ...
    // keyboard_func, mouse_click_func, and mouse_move_func are all defined in control.cpp
    // key callback "q" quits the program
    glutKeyboardFunc(keyboard_func);
    glutMouseFunc(mouse_click_func);
    // holding left button rotates, holding middle button translates the object
...
/*
 * C: parseArgs goes through the flags and input arguments and sets the parameters for the raytracer accordingly.
 */
void parseArgs (int argc, char** argv)
{
    ...
    while (i < argc)
    {
        ...
        case 's':               // [ -s source rayfile ]
            if (++i == argc) usage();
            options->source = std::string(argv[i]);
            break;

        case 'd':               // [ -d destination image ]
            if (++i == argc) usage();
            options->destination = std::string(argv[i]);
            break;
...
void display ()
{
    // Why was the following code commented out? Was it intentionally done? If not, should we uncomment it?
