...
void RayFile::glDraw ()
{
    ...
    {
      // In what way does this approximate translucent behavior? Why are we doing a two pass render?
    /*
...
void RayFile::setFile (std::string file_)
{
    file = file_;
    setDir (NULL);
}
...
void RayFile::setDir (const char* relativeTo)
{
    ...
    bool done = false;
    // What is i? What is this for loop for/what does it do?
    int i;
    ...
    if (i > 0)
    {
        ...
        char filecstr[MAX_PATH];
        strncpy(filecstr, file.c_str(), MAX_PATH);
        strncat(dir, filecstr, i+1);

        char temp[MAX_PATH];
        strncpy(temp, filecstr, MAX_PATH);
        memset(filecstr, 0, MAX_PATH);
        strncpy(filecstr, temp+i+2, strlen(temp+i+2));
        file = string(filecstr);
    }
}
...
istream& RayFile::read (istream& in)
{
    ...
    while (in && !done)
    {
        ...
        if (strncmp(next, "#version", 8) == 0)
        {
            ...
            if (strncmp(next, VersionTag17, strlen(VersionTag17)) == 0)
            {
                done = true;
                strcpy(version, VersionTag17);
            }
            else
            {
    ...
    while (in && !done)
    {
        ...
        if (in && next[0] == '#')
        {
            ...
            else if (strncmp(next, "#material", 8) == 0)
            {
                ...
                material->setMatDirectory(dir.c_str());
                in >> *material;
...
{
    out << "#version HMCCS155FALL2017" << endl << endl;
...
istream& RayFile::includeRayFile (istream& in)
{
    ...
    char buf1[MAX_NAME];
    char buf2[MAX_PATH];
    Flag flags[2] = { { (char*)"-n", STRING, true, MAX_NAME, buf1},
                      { (char*)"-f", STRING, true, MAX_PATH, buf2}
                    };

    rayfile->name = string(buf1);
    rayfile->file = string(buf2);
    ...
    rayfile->setDir(dir.c_str());
    char filename[MAX_PATH];

    strncpy(filename, rayfile->dir.c_str(), MAX_PATH);
    strncat(filename, "/", 2);
    strncat(filename, rayfile->file.c_str(), MAX_PATH);
