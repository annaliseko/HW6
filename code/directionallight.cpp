...
Color3d DirectionalLight::getDiffuse (Intersection& info)
{
   ...
   */
   ...
   std::cout << "getting diffuse " << std::endl;
    ...
    if (angleFactor<0)
      ...
      std::cout << "i'm gonna die" << std::endl;
        Color3d sup = color * info.material->getDiffuse(info) * angleFactor;
        std::cout << "i have died??" << std::endl;
        return sup;
...
bool DirectionalLight::getShadow (Intersection& iInfo, ShapeGroup* root)
{
   ...
   */
    ...
    std::cout << "shadow ray: " << tmpInfo.theRay << std::endl;
