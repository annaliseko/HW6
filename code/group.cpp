...
double Group::intersect (Intersection& intersectionInfo)
{
    ...
    */
    ...
    for (VECTOR(ShapeGroup*)::iterator sg = list.begin(); sg != list.end(); ++sg)
    {
        ...
        std::cout << "tempbefore: " << tempInfo.theRay << std::endl;
        // Distance for the current object
        double currDist = (*sg)->intersect(tempInfo);
        std::cout << "tempafter: " << tempInfo.theRay << std::endl;

        // RAY_CASTING TODO (Intersection)
        // In case of a new closest intersection, update alpha and localInfo
        // RAY_CASTING TODO (Intersection)
        if(alpha > 0 || currDist > 0)
            std::cout << "alpha: " << alpha << " currDist: " << currDist << std::endl;
        if (alpha == -1) 
        {
            if(currDist >= 0)
            {
        ...
        else if (currDist < alpha && currDist >= 0)
        {
            alpha = currDist;
            localInfo.theRay = tempInfo.theRay;
            std::cout << "temp ray: " << tempInfo.theRay << std::endl;
        }

    }

    intersectionInfo.theRay = localInfo.theRay;
    std::cout << "ray is: " << intersectionInfo.theRay << std::endl;
...
istream& Group::read (istream& in)
{
    ...
    //if(strcmp(file->version, VersionTag17) == 0)
    //  in >> transform;
    if(strcmp(file->version, VersionTag17) == 0)
